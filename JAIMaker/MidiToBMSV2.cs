...
static class MidiToBMSV2
{
    ...
    public static void doToBMS(MidiSequence wtf, Stream outFile)
    {
        var largest_delta = 0;
        ...
        {
            JaiWriter.Write(JaiSeqOpcodeV2.OpenTrack); // Write dummy open track commands
            JaiWriter.Write((byte)i); // Index
            writeInt24BE(JaiWriter, 0); // 0-size pointer.

        }

        JaiWriter.Write(JaiSeqOpcodeV2.RegLoad); // Tell JAI to set the timebase
        JaiWriter.Write((byte)0x62); // timebase register
        JaiWriter.Write((short)wtf.TicksPerBeatOrFrame); // Write timebase value
        JaiWriter.Write(JaiSeqOpcodeV2.Tempo); // Tell JAI to write the tempo
        ...
        JaiWriter.Write(JaiSeqOpcodeV2.WaitByte); // Wait
        ...
        {
            JaiWriter.Write(JaiSeqOpcodeV2.Jmp); // Jump to position
            writeInt24BE(JaiWriter, (int)rootTrack_JumpPos); // int24 position.
        }

        JaiWriter.Write(JaiSeqOpcodeV2.Finish); // Write the finisher for the track, not because it's required but just becase it's standard.
        ...
        for (int TrackID = 0; TrackID < wtf.Tracks.Count; TrackID++)
        {
            ...
            JaiWriter.Write(JaiSeqOpcodeV2.Bank);
            JaiWriter.Write((byte)Root.instrumentBanks[TrackID]);

            JaiWriter.Write(JaiSeqOpcodeV2.Prg);
            ...
            JaiWriter.Write(JaiSeqOpcodeV2.Finish);
        ...
        for (int i = 0; i < wtf.Tracks.Count; i++)
        {
            JaiWriter.Write(JaiSeqOpcodeV2.OpenTrack); // Write open track command
            JaiWriter.Write((byte)i); // Index
            writeInt24BE(JaiWriter, TrackAddresses[i]);
        }

        JaiWriter.Flush();
        outFile.Write(ReadToEnd(JaiTrackFinal));
    }
    ...
    public static void writeDelta(BeBinaryWriter JaiWriter, int delta)
    {
        ...
        {
            JaiWriter.Write(JaiSeqOpcodeV2.WaitByte); // 8 bit wait command
            JaiWriter.Write((byte)delta);  // write delta
        }
        else
        {
            JaiWriter.Write(JaiSeqOpcodeV2.Wait);
            JaiWriter.WriteVarInt(delta);
        }
    }
